...
file at a time with this plugin and ``CAPTURE_OUT`` naming the JSON output::
    ...
    for f in trig hyperbolic inverse power_exp_log complex_parts integer_funcs \
        ...
        PYTHONPATH=.:tools:/path/to/sympy python -m pytest -q -p no:cacheprovider \
            -p refine_battery_capture tests/refine_v3/test_$f.py
    done

then ``python tools/refine_battery_generate.py /tmp/cap tests/refine_identities/battery_v3.py``.

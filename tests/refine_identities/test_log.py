...
ROWS = [  # (input, assumptions, team output or None, relation)
    ...
    (log(x**2), Q.real(x), 2*log(Abs(x)), "miss: row domain ~Q.zero(b) blocks it; the engine resolves it once the power row's domain is ~Q.zero(b) | ~Q.zero(e) (see test_engine.py)"),
    ...
    (log(x*y), Q.positive(x) & Q.complex(y), log(x) + log(y), "miss: row domain ~Q.zero(r) blocks it; the engine resolves it once the product row's domain is true (see test_engine.py)"),

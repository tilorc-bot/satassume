...
def test_matrix_pattern_binds_an_atom_and_its_shape():
    ...
    rule = compile_rule(Transpose(Z), ZeroMatrix(q, m), Q.zero(Z))
    assert rule(Transpose(B), Q.zero(B)) == ZeroMatrix(3, 2)
    sym = compile_rule(Transpose(A), A, Q.symmetric(A))
...
def test_one_term_and_the_rest_of_a_matrix_sum_or_hadamard_product():
    ...
    rule = compile_rule(Z + R, R, Q.zero(Z))
    assert rule(X + Y + W2, Q.zero(X)) == Y + W2
    had = compile_rule(HadamardProduct(Z, R), ZeroMatrix(m, q), Q.zero(Z))
...
def test_scalar_factor_and_the_rest_of_a_product():
    ...
    rule = compile_rule(c*Z, ZeroMatrix(m, q), Q.zero(c))
...
def test_adjacent_run_in_a_product():
    ...
    orth = compile_rule(A*Transpose(A), Identity(m), Q.orthogonal(A))
    assert orth(2*X*X.T*Y, Q.orthogonal(X)) == 2*Y
    zero = compile_rule(Z*W, ZeroMatrix(m, s), Q.zero(Z))
    assert zero(B*C, Q.zero(B)) == ZeroMatrix(2, 2)
    uni = compile_rule(Adjoint(A)*A, Identity(m), Q.unitary(A))

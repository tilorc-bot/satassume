"""Record the ``ask`` query stream that satrefine issues over its battery.
    ...
    PYTHONHASHSEED=0 SATREFINE_HANDLERS=handlers_identities SATREFINE_IDENTITIES=generated \
      PYTHONPATH=.:/path/to/sympy python -m satrefine.tools.refine_record stream.pkl
...
``satrefine/tools/refine_replay.py`` against any satassume checkout.
"""
import argparse, pickle, time
_ap = argparse.ArgumentParser(description=__doc__.split("\n\n")[0])
_ap.add_argument("out", help="the pickle to write")
...
with open(_args.out, "wb") as f: pickle.dump(stream, f)

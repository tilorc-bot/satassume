...
Usage::
    ...
    PYTHONPATH=.:/path/to/sympy .venv/bin/python -m satrefine.tools.refine_identity_scoreboard
...
def parse() -> argparse.Namespace:
    ...
    p.add_argument("--fuzz", nargs="*", metavar="SEED CASES",
                   help="also run satrefine/tools/refine_fuzz.py; with no values a smoke run (seed 2, 200 cases)")
...
def main() -> None:
    ...
    if "satrefine" in sys.modules:           # python -m: satrefine is loaded already
        ...
        rerun_with(handlers=args.handlers)

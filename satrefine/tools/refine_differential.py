...
The inputs are exactly those of ``satrefine/tools/refine_fuzz.py`` (its expression
...
def fz():
    """``satrefine/tools/refine_fuzz.py`` as a module (imports satrefine: set the package first)."""
    global _FZ
    if _FZ is None:
        ...
        try:
            _FZ = importlib.import_module("satrefine.tools.refine_fuzz")
        finally:
            sys.argv = saved
    return _FZ
